...
function getAllDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var result = isLate(allDeadline.duedate, allDeadline.duetime)
            .toString();
        if (result == "true") {
            $('#allList')
                .append('<li id = "' + allDeadline.duedate + ' ' + allDeadline.duetime + '"><a href="#DeadlineDetail" id = "' +
...
function getHomeworkDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (result == "true") {
            ////alert('append');	
            $('#homeworkList')
                .append('<li id = "' + homeworkDeadline.duedate + ' ' + homeworkDeadline.duetime +
                    '"><a href="#DeadlineDetail" id = "' + homeworkDeadline.id + '" data-transition = "slide">' +
                    homeworkDeadline.class + '<br>' + homeworkDeadline.duedate + '    ' +
                    homeworkDeadline.duetime + '<br>' + homeworkDeadline.description + '</a></li>');
        }

    }
    var elems = $('#homeworkList')
        .children('li')
        .remove();
...
function getTestDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var result = isLate(testDeadline.duedate, testDeadline.duetime)
            .toString();
        if (result == "true") {
            $('#testList')
                .append('<li id = "' + testDeadline.duedate + ' ' + testDeadline.duetime +
                    '"><a href="#DeadlineDetail" id = "' + testDeadline.id + '" data-transition = "slide">' +
                    testDeadline.class + '<br>' + testDeadline.duedate + '    ' + testDeadline.duetime + '<br>' +
                    testDeadline.description + '</a></li>');
        }
    }
    $(function () {
        var elems = $('#testList')
...
function getAllFinishedDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        $('#allFinishedList')
            .prepend('<li id = "' + allFinishedList.duedate + ' ' + allFinishedList.duetime +
                '"><a href="#DeadlineDetail" id = "' + allFinishedDeadline.id + '" data-transition = "slide"><del>' +
                allFinishedDeadline.class + '<br>' + allFinishedDeadline.duedate + '  ' +
                allFinishedDeadline.duetime + '<br>' + allFinishedDeadline.description + '</del></a></li>');

    }
    var elems = $('#allFinishedList')
        .children('li')
        .remove();
...
function getHomeworkFinishedDeadlines_success(tx, results) {
    ...
    var len = results.rows.length;
    $('#homeworkFinishedList')
        .empty();
    for (var i = 0; i < len; i++) {
        var homeworkFinishedDeadline = results.rows.item(i);

        $('#homeworkFinishedList')
            .prepend('<li id = "' + homeworkFinishedList.duedate + ' ' +
                homeworkFinishedList.duetime + '"><a href="#DeadlineDetail" id = "' + homeworkFinishedDeadline.id +
                '" data-transition = "slide"><del>' + homeworkFinishedDeadline.class + '<br>' +
                homeworkFinishedDeadline.duedate + '    ' + homeworkFinishedDeadline.duetime + '<br>' +
                homeworkFinishedDeadline.description + '</del></a></li>');

    }
    var elems = $('#homeworkFinishedList')
        .children('li')
        .remove();
...
function getTestFinishedDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        $('#testFinishedList')
            .prepend('<li id = "' + testFinishedList.duedate + ' ' + testFinishedList.duetime +
                '"><a href="#DeadlineDetail" id = "' + testFinishedDeadline.id + '" data-transition = "slide"><del>' +
                testFinishedDeadline.class + '<br>' + testFinishedDeadline.duedate + '    ' +
                testFinishedDeadline.duetime + '<br>' + testFinishedDeadline.description + '</del></a></li>');

    }
    var elems = $('#testFinishedList')
        .children('li')
        .remove();
...
function getAllMissedDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var result = isLate(allMissedDeadline.duedate, allMissedDeadline.duetime)
            .toString();
        if (result == "false") {
            $('#allMissedList')
                .prepend('<li id = "' + allMissedList.duedate + ' ' + allMissedList.duetime +
                    '"><a href="#DeadlineDetail" id = "' + allMissedDeadline.id + '" data-transition = "slide">' +
                    allMissedDeadline.class + '<br>' + allMissedDeadline.duedate + '  ' + allMissedDeadline.duetime +
                    '<br>' + allMissedDeadline.description + '</a></li>');
        }
    }
    var elems = $('#allMissedList')
        .children('li')
        .remove();
...
function getHomeworkMissedDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var result = isLate(homeworkMissedDeadline.duedate, homeworkMissedDeadline.duetime)
            .toString();
        ////alert('result: ' + result);
        if (result == "false") {
            ////alert('append');				
            $('#homeworkMissedList')
                .prepend('<li id = "' + homeworkMissedList.duedate + ' ' + homeworkMissedList.duetime +
                    '"><a href="#DeadlineDetail" id = "' + homeworkMissedDeadline.id + '" data-transition = "slide">' +
                    homeworkMissedDeadline.class + '<br>' + homeworkMissedDeadline.duedate + '    ' +
                    homeworkMissedDeadline.duetime + '<br>' + homeworkMissedDeadline.description + '</a></li>');
        } else continue;;
    }
    var elems = $('#homeworkMissedList')
        .children('li')
        .remove();
...
function getTestMissedDeadlines_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        //Compare with current time
        var result = isLate(testMissedDeadline.duedate, testMissedDeadline.duetime)
            .toString();

        if (result == "false") {
            $('#testMissedList')
                .prepend('<li id = "' + testMissedList.duedate + ' ' + testMissedList.duetime + '">' +
                    +'<a href="#DeadlineDetail" id = "' + testMissedDeadline.id + '" data-transition = "slide">' +
                    testMissedDeadline.class + '<br>' + testMissedDeadline.duedate + '    ' + testMissedDeadline.duetime +
                    '<br>' + testMissedDeadline.description + '</a></li>');
        }
    }
    var elems = $('#testMissedList')
        .children('li')
        .remove();
...
function getFormInfo() {
    ////alert(id);
    var description = document.getElementById("shortDescription")
        .value;
    ////alert(description);
    var classDeadline = document.getElementById("class")
        .value;
    ////alert(classDeadline);
...
function saveDeadlineToDB() {
    ...
    //alert(dbId);
    var dbDescription = document.getElementById("shortDescriptionAddNew")
        .value;
    //alert(dbDescription);
    var dbClass = document.getElementById("classAddNew")
        .value;
    //alert(dbClass);
    var dbDueDate = document.getElementById("dueDateAddNew")
        .value;
    //alert(dbDueDate);
    var dbDueTime = document.getElementById("dueTimeAddNew")
        .value;
    //alert(dbDueTime);
    var dbType = document.getElementById("typeAddNew")
        .value;
    //alert(dbType);
    var dbAdditionalInfo = document.getElementById("additionalInfoAddNew")
        .value;
    //alert(dbAdditionalInfo);
    var dbFinished = document.getElementById("finishedAddNew")
        .value;
    //alert(dbFinished);
...
function getClasses_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        $('#class')
            .append('<option value="' + classDB.name + '">' + classDB.name + '</option>');
        $('#classAddNew')
            .append('<option value="' + classDB.name + '">' + classDB.name + '</option>');
        $('#classList')
            .append('<li><a href="#classDetail" id = "' + classDB.id + '" data-transition = "slide" >' + classDB.name + '</a></li>');
    }
    $("#classList")
        .listview()
        .listview('refresh');
...
function getClassDetail_success(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        document.getElementById("className")
            .value = name;
        document.getElementById("classLocation")
            .value = location;
        document.getElementById("classTime")
            .value = time;
        document.getElementById("classTeacher")
            .value = teacher;
        document.getElementById("classTeacherEmail")
            .value = email;
        document.getElementById("classTeacherPhone")
            .value = phone;

        if (date === 'Monday') {
            document.getElementById("classDate")
                .selectedIndex = "0";
...
function getClassFormInfo() {
    ...
    var name = document.getElementById("className")
        .value;
    //alert(name);
    var location = document.getElementById("classLocation")
        .value;
    //alert(location);
    var date = document.getElementById("classDate")
        .value;
    //alert(date);
    var time = document.getElementById("classTime")
        .value;
    //alert(time);
    var teacher = document.getElementById("classTeacher")
        .value;
    //alert(teacher);
    var email = document.getElementById("classTeacherEmail")
        .value;
    //alert(email);
    var phone = document.getElementById("classTeacherPhone")
        .value;
    //alert(phone);
...
function saveClassToDB() {
    ...
    //alert(dbId);
    var dbName = document.getElementById("newClassName")
        .value;
    //alert(dbName);
    var dbLocation = document.getElementById("newClassLocation")
        .value;
    //alert(dbLocation);
    var dbDate = document.getElementById("newClassDate")
        .value;
    //alert(dbDate);
    var dbTime = document.getElementById("newClassTime")
        .value;
    //alert(dbTime);
    var dbTeacher = document.getElementById("newClassTeacher")
        .value;
    //alert(dbTeacher);
    var dbEmail = document.getElementById("newClassTeacherEmail")
        .value;
    //alert(dbEmail);
    var dbPhone = document.getElementById("newClassTeacherPhone")
        .value;
    //alert(dbPhone);
